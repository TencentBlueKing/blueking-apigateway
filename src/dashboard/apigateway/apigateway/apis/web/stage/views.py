...
class StageStatusUpdateApi(StageQuerySetMixin, generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if data["status"] == StageStatusEnum.INACTIVE.value:
            MCPServerHandler.disable_servers(gateway_id=request.gateway.id, stage_id=instance.id, username=username)
...
class ProgrammableStageDeployRetrieveApi(StageQuerySetMixin, generics.RetrieveUpdateAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        context_data.update(stage_deploy_result)
        output_slz = self.get_serializer(
            instance=stage_deploy_result.get("last_deploy_history", ProgrammableGatewayDeployHistory()),
