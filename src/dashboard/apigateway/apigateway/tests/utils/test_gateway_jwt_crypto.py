...
from apigateway.utils.crypto import BkCrypto, CustomCrypto, get_crypto
...
class TestGetCrypto:
    def test_get_crypto_default(self, settings):
        crypto = get_crypto()
        assert isinstance(crypto, CustomCrypto)

    def test_get_crypto_not_default(self, settings):
        settings.BK_CRYPTO_TYPE = "CLASSIC"

        crypto = get_crypto()
        assert isinstance(crypto, BkCrypto)

    def test_get_crypto_unknown(self, settings):
