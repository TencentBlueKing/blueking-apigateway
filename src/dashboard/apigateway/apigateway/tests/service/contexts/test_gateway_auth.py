...
class TestGatewayAuthContext:
    ...
    def test_get_gateway_id_to_auth_config(self, fake_gateway, allow_update):
        GatewayHandler.save_auth_config(fake_gateway.id, "default", allow_update_api_auth=allow_update)
        ...
        assert result[fake_gateway.id].allow_update_gateway_auth is allow_update

    @pytest.mark.parametrize("allow_update", [True, False])
    def test_get_auth_config(self, fake_gateway, allow_update):
        GatewayHandler.save_auth_config(fake_gateway.id, "default", allow_update_api_auth=allow_update)

        result = GatewayAuthContext().get_auth_config(fake_gateway.id)
        assert result.gateway_type == GatewayTypeEnum.CLOUDS_API.value
        assert result.allow_update_gateway_auth is allow_update
