...
class TestMCPServerRetrieveApi:
    ...
    def test_retrieve_returns_tool_schema(self, request_view, fake_gateway, mocker):
        ...
        mocker.patch(
            "apigateway.apis.v2.mcp_server.ResourceVersionHandler.get_resource_id_to_schema_by_resource_version",
            return_value={101: fake_schema},
        )
        mocker.patch(
            "apigateway.apis.v2.mcp_server.Release.objects.filter",
    ...
    def test_retrieve_returns_empty_schema_when_no_release(self, request_view, fake_gateway, mocker):
        """测试无 Release 时工具 schema 返回空对象"""
        fake_gateway.status = GatewayStatusEnum.ACTIVE.value
        fake_gateway.maintainers = ["admin"]
        fake_gateway.save()
        ...
        tool_resource = mock.MagicMock()
        tool_resource.id = 201
        tool_resource.name = "tool1"
        tool_resource.description = "Tool 1"
        tool_resource.method = "GET"
        tool_resource.path = "/api/v1/tool1/"
        tool_resource.verified_user_required = False
        tool_resource.verified_app_required = True
        tool_resource.resource_perm_required = True
        ...
        mocker.patch(
            "apigateway.apis.v2.mcp_server.MCPServerHandler.get_tools_resources_and_labels",
            return_value=([tool_resource], {201: []}),
        )
        mocker.patch(
            "apigateway.apis.v2.mcp_server.MCPServerHandler.get_prompts_count_map",
            return_value={mcp_server.id: 0},
