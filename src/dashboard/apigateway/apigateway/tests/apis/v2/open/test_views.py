...
class TestItsmCallbackApi:
    ...
    def test_fallback_to_verify_token_from_query(self, request_to_view, request_factory, settings, mocker):
        ...
        mock_handle = mocker.patch("apigateway.apis.v2.open.itsm_views.ItsmCallbackApi._handle_approval_result")

        request = request_factory.post(
            "/api/v2/open/itsm/callback/?verify_token=verify-token-001",
            data=self._build_callback_data(callback_token=""),
            format="json",
        )
        request.app = mock.MagicMock(app_code="bk-itsm4")

        response = request_to_view(request, view_name="openapi.v2.open.itsm.callback")
    ...
    def test_reject_when_no_callback_token_and_verify_token(self, request_view, settings, mocker):
        settings.BK_ITSM4_CALLBACK_ALLOWED_APP_CODES = ["bk-itsm4"]
        mocker.patch("apigateway.apis.v2.open.itsm_views.ItsmCallbackApi._handle_approval_result")

        response = request_view(
            method="POST",
            view_name="openapi.v2.open.itsm.callback",
    ...
    def test_require_tenant_header_in_multi_tenant_mode(self, request_view, settings, mocker):
    ...
    def test_accept_tenant_header_in_multi_tenant_mode(self, request_view, settings, mocker):
        settings.ENABLE_MULTI_TENANT_MODE = True
        settings.BK_ITSM4_CALLBACK_ALLOWED_APP_CODES = ["bk-itsm4"]
        mock_handle = mocker.patch("apigateway.apis.v2.open.itsm_views.ItsmCallbackApi._handle_approval_result")
