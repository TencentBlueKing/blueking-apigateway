...
def test_search_chain_by_any_id_tries_id_fallbacks(mocker):
    ...
    result = search_chain_by_any_id("request-id")

    assert result["spans"]
    assert client_class.call_args_list == [
        call(request_id="request-id"),
        call(x_request_id="request-id"),
        call(upstream_request_id="request-id"),
    ]


def test_search_gateway_log_queries_by_request_id(mocker):
...
def test_upstream_request_fallback_queries_gateway_log_when_mcp_log_missing(mocker):
