...
from apigateway.biz.data_plane.data_plane import DataPlaneHandler
...
class TestResolveDataPlaneIdsByNames:
    def test_success(self):
        ...
        result = DataPlaneHandler.resolve_data_plane_ids_by_names(["plane-a", "plane-b"])
    ...
    def test_missing_raises_validation_error(self):
        ...
        with pytest.raises(ValidationError):
            DataPlaneHandler.resolve_data_plane_ids_by_names(["exists", "does-not-exist"])

    def test_deduplicates(self):
        dp = G(DataPlane, name="dup-plane", status=DataPlaneStatusEnum.ACTIVE.value)

        result = DataPlaneHandler.resolve_data_plane_ids_by_names(["dup-plane", "dup-plane"])
        assert result == [dp.id]
...
class TestGetDefaultDataPlaneId:
    def test_success(self):
        ...
        assert DataPlaneHandler.get_default_data_plane_id() == dp.id

    def test_not_found_raises(self):
        with pytest.raises(DataPlane.DoesNotExist):
            DataPlaneHandler.get_default_data_plane_id()
...
class TestGetTeBpSyncDataPlaneIds:
    ...
    def test_not_start_returns_default(self, settings, bp_data_plane):
        ...
        result = DataPlaneHandler._get_te_bp_sync_data_plane_ids(default_id)
...
class TestGetSyncDataPlaneIds:
    ...
    def test_with_explicit_names(self):
        ...
        result = DataPlaneHandler.get_sync_data_plane_ids("any-gateway", data_plane_names=["explicit-plane"])
        assert result == [dp.id]

    def test_default_fallback(self, settings):
        settings.EDITION = "ee"
        result = DataPlaneHandler.get_sync_data_plane_ids("my-gateway")
    ...
    def test_te_plugin_gateway_routing(self, settings):
        ...
        result = DataPlaneHandler.get_sync_data_plane_ids("bp-my-plugin")
        assert result == [self.default_dp.id]

    def test_te_non_plugin_gateway_uses_default(self, settings):
        settings.EDITION = "te"
        result = DataPlaneHandler.get_sync_data_plane_ids("regular-gateway")
        assert result == [self.default_dp.id]
