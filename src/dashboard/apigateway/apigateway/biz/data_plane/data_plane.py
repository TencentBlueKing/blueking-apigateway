...
class DataPlaneHandler:
    @staticmethod
    def get_default_data_plane_id() -> int:
        return DataPlane.objects.get_default().id

    @staticmethod
    def resolve_data_plane_ids_by_names(data_plane_names: List[str]) -> List[int]:
    ...
    def _get_te_bp_sync_data_plane_ids(default_data_plane_id: int) -> List[int]:
        ...
        gray_stage = getattr(
            settings, "BK_PLUGINS_DATA_PLANE_GRAY_STAGE", BkPluginsDataPlaneGrayStageEnum.NOT_START.value
        )
    ...
    def get_sync_data_plane_ids(gateway_name: str, data_plane_names: Optional[List[str]] = None) -> List[int]:
        if data_plane_names:
            return DataPlaneHandler.resolve_data_plane_ids_by_names(data_plane_names)

        default_data_plane_id = DataPlaneHandler.get_default_data_plane_id()
        if settings.EDITION == "te" and gateway_name.startswith(PLUGIN_GATEWAY_PREFIX):
            return DataPlaneHandler._get_te_bp_sync_data_plane_ids(default_data_plane_id)
