...
def get_data_differ_number(data: dict) -> int:
    ...
    if isinstance(data, dict) and "series" in data and isinstance(data["series"], list) and len(data["series"]) > 0:
        ...
        if len(datapoints) > 1:
            ...
            if last_value is None and len(datapoints) > 1:
                last_number = 0 if datapoints[-2][0] is None else datapoints[-2][0]
            else:
...
class IngressMetrics(BaseMetrics):
    ...
    def _get_query_promql(
        ...
        label_list = [
            *self.default_labels,
            ("type", "=", "ingress"),
            # service 的参数规则：{gateway_name}.{stage_name}.{stage_id}-{backend_id}
            ("service", "=", f"{gateway_name}.{stage_name}.{stage_id}-{backend.id}"),
        ]
        if resource_id:
