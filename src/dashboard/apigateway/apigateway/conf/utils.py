...
def get_default_feature_flags(
    ...
    return {
        ...
        "MENU_ITEM_ESB_API": esb_enabled and env.bool("FEATURE_FLAG_MENU_ITEM_ESB_API", True),
        # TODO: remove in the future, and remove in the helm-chart and te repo
        # 是否展示"组件 API 文档"菜单项
        "MENU_ITEM_ESB_API_DOC": esb_enabled and env.bool("FEATURE_FLAG_MENU_ITEM_ESB_API_DOC", True),
        # 是否将 ESB 数据同步到网关。需要考虑这个是否还需要
        "SYNC_ESB_TO_APIGW_ENABLED": esb_enabled and env.bool("FEATURE_FLAG_SYNC_ESB_TO_APIGW_ENABLED", True),
        # 网关编辑页，是否支持填写网关"绑定应用"
