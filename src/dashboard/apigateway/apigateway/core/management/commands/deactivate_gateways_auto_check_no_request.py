...
from apigateway.apps.metrics.models import StatisticsAPIRequestByDay
    ...
    def handle(self, file_path: str, dry_run: bool, **options):
        ...
        for gateway in gateways:
            ...
            if dry_run:
                ...
                # 1. check no requests from 2023-01-01 00:00:00
                if StatisticsAPIRequestByDay.objects.filter(
                    api_id=g.pk, start_time__gt="2023-01-01 00:00:00"
                ).exists():
                    print(f"skip deactivate gateway: {gateway['name']}, requests found from 2023-01-01 00:00:00")
                    continue
