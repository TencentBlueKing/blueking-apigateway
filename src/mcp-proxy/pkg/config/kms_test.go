...
func TestLoadKMSDisabled(t *testing.T) {
	for _, flag := range []string{"", "false", "False", "0", "on", "ok", "y", "yes", "1", "2", "-1", "TRUE", "tRuE", " true", "True "} {
		t.Run(flag, func(t *testing.T) {
			...
			if cfg.Databases[0].User != "old-user" ||
				cfg.DatabaseMap["apigateway"].Password != "old-password" {
...
func TestLoadKMSRequiredFields(t *testing.T) {
	paths := [][]string{
		{"mysql", "apigw", "username"},
		{"mysql", "apigw", "password"},
...
func TestLoadKMSCredentials(t *testing.T) {
	...
	if cfg.Databases[0].User != "new-user" || cfg.DatabaseMap["apigateway"].Password != "$p@ss:/?#%密码" ||
		cfg.DatabaseMap["audit"].Password != "audit-password" {
		t.Fatal("KMS credentials not applied to database list and map")
	}
	if cfg.DatabaseMap["apigateway"].Host != "mysql.example" || cfg.DatabaseMap["apigateway"].Name != "gateway" {
		t.Fatal("KMS changed non-credential database fields")
	}
	if cfg.McpServer.EncryptKey != "$new-encryption-key" || cfg.McpServer.CryptoNonce != "existing-nonce" ||
		cfg.PProf.Password != "untouched-pprof" {
		t.Fatal("KMS must override encryption env but preserve unrelated config")
	}
