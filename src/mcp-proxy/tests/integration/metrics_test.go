	...
	Describe("Streamable HTTP Metrics", func() {
		It("should report mcp_requests_total after initialize and tools/call", func() {
			...
			Expect(
				containsMetricLine(
					metricsOutput,
					"bk_apigateway_mcp_proxy_mcp_requests_total",
					map[string]string{
				...
				BeTrue(),
				"should have mcp_requests_total for initialize",
			)
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_requests_total for tools/call",
			)
		})
		...
		It("should report mcp_tool_calls_total with tool name", func() {
			...
			Expect(
				containsMetricLine(
					metricsOutput,
					"bk_apigateway_mcp_proxy_mcp_tool_calls_total",
					map[string]string{
						"gateway_name":    "bk-apigateway",
						"mcp_server_name": "test-http-server",
						"tool_name":       "echo",
				...
				BeTrue(),
				"should have mcp_tool_calls_total for echo tool",
			)
		})
		...
		It("should report mcp_sessions_active after initialize", func() {
			...
			Expect(
				containsMetricLine(
					metricsOutput,
					"bk_apigateway_mcp_proxy_mcp_sessions_active",
					map[string]string{
						"gateway_name":    "bk-apigateway",
						"mcp_server_name": "test-http-server",
						"app_code":        "test-app",
				...
				BeTrue(),
				"should have mcp_sessions_active for test-http-server",
			)
		})
		...
		It("should report mcp_errors_total when calling non-existent tool", func() {
			...
			Expect(
				containsMetricLine(
					metricsOutput,
					"bk_apigateway_mcp_proxy_mcp_errors_total",
					map[string]string{
						"gateway_name":    "bk-apigateway",
						"mcp_server_name": "test-http-server",
						"method":          "tools/call",
				...
				BeTrue(),
				"should have mcp_errors_total for failed tools/call",
			)
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_requests_total with error for failed tools/call",
			)
		})
	})
	...
	Describe("SSE Metrics", func() {
		It("should report mcp metrics for SSE protocol", func() {
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_requests_total for SSE initialize",
			)
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_requests_total for SSE tools/call",
			)
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_tool_calls_total for SSE echo tool",
			)
			...
			Expect(
				...
				BeTrue(),
				"should have mcp_sessions_active for test-sse-server",
			)
		})
	})
	...
	Describe("Metrics Gateway Name Label", func() {
		It("should include gateway_name label in metrics", func() {
			...
			Expect(
				...
				BeTrue(),
				"should have gateway_name=bk-apigateway in mcp_requests_total",
			)
		})
	})
})
