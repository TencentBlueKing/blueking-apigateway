...
const getFallbackURL = () => {
  ...
  if (is(`.${window.GLOBAL_CONFIG.OA_DOMAIN}`)) {
    return `//open.${window.GLOBAL_CONFIG.OA_DOMAIN}/static_api/v3/components_vue/2.0/member-selector-fallback.html`
  }
  if (is(`.${window.GLOBAL_CONFIG.WOA_DOMAIN}`)) {
    return `//bkrepo.${window.GLOBAL_CONFIG.WOA_DOMAIN}/generic/bkcdn/static/bk-magicbox/member-selector-fallback.html`
  }
  if (is(`.${window.GLOBAL_CONFIG.IED_DOMAIN}`)) {
    return `//o.${window.GLOBAL_CONFIG.IED_DOMAIN}/static_api/v3/components_vue/2.0/member-selector-fallback.html`
  }
}
