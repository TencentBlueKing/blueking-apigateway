  ...
  <div class="data-box wrapper">
    ...
    <div class="row-item mb10">
      ...
      <div class="value">
        ...
        <i class="doc-copy vm icon-hover apigateway-icon icon-ag-copy ag-doc-icon" v-if="params.sdk_download_url" v-bk-tooltips="$t('复制')" :data-clipboard-text="params.sdk_download_url"></i>
        <i class="ag-doc-icon doc-download-line vm icon-hover apigateway-icon icon-ag-download-line" v-if="params.sdk_download_url" v-bk-tooltips="$t('下载')" @click="handleDownload"></i>
      </div>
    </div>
    ...
    <div class="row-item mb10">
      ...
      <div class="value">
        ...
        <i class="ag-doc-icon doc-copy vm icon-hover apigateway-icon icon-ag-copy" v-if="params.sdk_install_command" v-bk-tooltips="$t('复制')" :data-clipboard-text="params.sdk_install_command"></i>
      </div>
    </div>
    ...
    <template v-if="isApigw">
      <div class="row-item mb10">
        ...
        <div class="value">
          <span class="column-value" v-bk-tooltips.top="{ content: params.resource_version_display, allowHTML: false }">{{params.resource_version_display || '--'}}</span>
        </div>
      </div>
...
<script>
  export default {
    ...
    computed: {
      stageText () {
        let texts = []
        if (this.params.released_stages) {
          texts = this.params.released_stages.map(item => item.name)
        }
    ...
    methods: {
      handleDownload () {
        if (this.params.sdk_download_url) {
          window.open(this.params.sdk_download_url)
        }
      }
    }
  }
