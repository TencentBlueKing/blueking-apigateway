...
var (
	...
	// NOTE: 权限 - 缓存的变更来源
	// part1: 权限本身的操作
	// 删除/新增权限，此时 permission cache 中数据不是最新
	// 1. 新增权限 (频率高)
	//    1.1 如果之前没有访问过，立刻生效
	//    1.2 如果最近 1 分钟 访问过了 (提示无权限), 1 分钟后生效
	// 2. 删除权限 (频率低)
	//    2.1 如果最近 1 分钟没有访问过，立刻生效
	//    2.2 如果最近 1 分钟有访问过 (有权限), 1 分钟后生效

	// part2: 发布带来 release 的差异 (会导致最终取权限的 resource_id 有差异)
	// 1. 发布新的 release 到某个 stage (频率高)
	//    1.1 release 如果没有资源增删，不会影响权限
	//    1.2 release 如果删除了资源，资源不存在直接 404, 不会到权限判断 (此时权限缓存可能还在，1 分钟)
	//    1.3 release 如果新增了资源，原来没有访问过，立刻生效; 原来访问过会 404, 不会到生成缓存这一步，没有缓存，不影响权限判断
	// 2. 更改了 stageName (正常只在开发调试阶段存在，小概率)
	//    2.1 改完，发布，权限不会受影响 (因为 stageName 改了，不会命中 cache)
	//    2.2 改完，发布，访问，再改回原来的名字，再发布 (小概率): 因为 5 分钟内老的 stageName 后的缓存中，会导致查到 release-resource_id 可能有差异
	...
	// 4. 如果过期前的 1 分钟点击过在线调试，过期后，因为有缓存，会提示已过期 (直到 1 分钟后捞取 db 新的数据) (小概率) => 如果 0 处理了，不会出现
	// 结论：不处理，后续通过某些发布订阅解决

	// NOTE: 决策:(需要约定，并且用户需要感知到生效的延时是多久)
	// 1. 新增/删除权限，大部分场景立即生效，最多 1 分钟后生效
	// 2. 更改 stageName 发布 - 访问 - 再改回去 - 再发布，最多 5 分钟生效
	// 3. 在线调试，如果正好在过期前的 1 分钟 - 过期后 1 分钟，会提示无权限 (如果提前 5 分钟续期，不会出现)

	// NOTE: 这样，apigateway-core 强依赖 db, db 不可用会导致 `流量转发不可用`
