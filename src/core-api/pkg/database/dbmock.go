...
func NewMockSqlxDB() (*sqlx.DB, sqlmock.Sqlmock) {
	...
	if err != nil {
		logging.GetLogger().Errorf("An error was not expected when opening a stub database connection: %s", err)
	}
...
func RunWithMock(t *testing.T, test func(db *sqlx.DB, mock sqlmock.Sqlmock, t *testing.T)) {
	...
	if err := mock.ExpectationsWereMet(); err != nil {
		logging.GetLogger().Error("there were unfulfilled expectations")
	}
}
